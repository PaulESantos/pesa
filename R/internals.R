.onAttach <- function(lib, pkg) {
  packageStartupMessage(
    "This is pesa ",
    utils::packageDescription("pesa", fields = "Version"),
    appendLF = TRUE
  )
}
...
.vfoi_check_stem <- function(
  ...
  if (native.status | cultivated | natives.only | collection.info) {
    join <- " JOIN view_full_occurrence_individual ON (analytical_stem.taxonobservation_id  = view_full_occurrence_individual.taxonobservation_id)"
  } else {
    join <- ""
  }
...
check_internet <- function() {
  tryCatch(
    class(httr::GET("http://www.google.com/")) == "response",
    error = function(e) {
      return(FALSE)
    }
  )
